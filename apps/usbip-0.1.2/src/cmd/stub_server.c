...
static int recv_request_import(int sockfd)
{
	...
	if(!error) {
		...
		log_to_web(LOG_INFO, "PrintServer have replied to client.");
	}

	return 0;
}

static void usbip_refresh_waiting_list()
{
	if (devlist == NULL)
		return;
	...
	for(tmp = devlist; tmp != NULL; )		/* delete absent devices */
	{
		dlist_for_each_data(StubDriver->edev_list, edev, struct usbip_exported_device)
		{
			...
			{
				/*printf("%s %d tmp->busid = %s \n",__FUNCTION__,__LINE__, tmp->busid);*/
				absent = 0;
				status = edev->status;
				goto out;
			}
	...
	for(tmp = devlist; tmp != NULL; tmp = tmp->next)
	{
		if (tmp->status != SDEV_ST_AVAILABLE)
			continue;
		cli = tmp->client;
		/*printf("%s %d  tmp->client->hostname = %s \n",__FUNCTION__,__LINE__, cli->hostname);*/
		if (cli == NULL)
			return;
 ...
 * RETURN          :       N/A
 ***************************************************************************/
static void usbip_device_state()
{
	...
	if (!isBusy_now && state != SDEV_ST_AVAILABLE)
		isBusy_now = 1;
	else if (isBusy_now && state == SDEV_ST_AVAILABLE)
		isBusy_now = 0;
	else 
		return ;
 ...
 * RETURN          :       N/A
 ***************************************************************************/
 static uint32_t usbip_fill_bcastpkt()
{
...
static int listen_all_addrinfo(struct addrinfo *ai_head, int lsock[], int *maxfd)
{
	...
	*maxfd = -1;		/* maximum descriptor number */
	...
	#if 0
		/* add by tf ,110505, for debug */
		printf("-^_^- lsock[%d] = %d  -^_^-\n", n, lsock[n]);
		printf("-^_^- FD_SETSIZE = %d  -^_^-\n", FD_SETSIZE);
...
static void do_standalone_mode(void)
{
	...
	if(!usbip_use_debug)
		daemon_init();
	/* waiting for usbip module up, code hold  temporary  in spite of not necessary, tf */
	/*sleep(1);*/
	set_signal();

	/* modified by tf, 110505, imply we use  Internet IP Protocol to communicate with client. */
#if 1
	...
	close(sockfd);
#else
	char arg_ip[] = "192.168.1.111";
	inet_pton(AF_INET, arg_ip, &servaddr.sin_addr);
#endif
	sockfd = socket(AF_INET, SOCK_DGRAM, 0);
#if 1	/* for broadcast */
