/*
 * $Id: usbip_common.c 101 2006-05-16 13:52:41Z taka-hir $
 *
...
void usbip_dump_header(struct usbip_header *pdu)
{
	...
	switch(pdu->base.command) {
		...
		case USBIP_RET_SUBMIT:
			udbg("RET_SUBMIT: st %d x_flags %u al %u bw %u sf %d ec %d\n",
					pdu->u.ret_submit.status,
					pdu->u.ret_submit.transfer_flags,
					pdu->u.ret_submit.actual_length,
					pdu->u.ret_submit.bandwidth,
					pdu->u.ret_submit.start_frame,
					pdu->u.ret_submit.error_count);
		case USBIP_RET_UNLINK:
			udbg("RET_UNLINK: status %ld\n", pdu->u.ret_unlink.status);
			break;
		default:
			//* NOT REACHED *
			udbg("UNKNOWN\n");
...
static void usbip_pack_submit_pdu(struct usbip_header *pdu, struct urb *urb, int pack)
{
	if (pack) {
		/* vhci_tx.c */
		pdu->u.cmd_submit.transfer_flags		 = urb->transfer_flags & ~URB_NO_TRANSFER_DMA_MAP & ~URB_NO_SETUP_DMA_MAP;
		pdu->u.cmd_submit.transfer_buffer_length	 = urb->transfer_buffer_length;
		/* pdu->u.cmd_submit.bandwidth			 = urb->bandwidth; */
		pdu->u.cmd_submit.start_frame		 = urb->start_frame;
		pdu->u.cmd_submit.number_of_packets		 = urb->number_of_packets;
		pdu->u.cmd_submit.interval			 = urb->interval;
	} else  {
		/* stub_rx.c */
		urb->transfer_flags         = pdu->u.cmd_submit.transfer_flags;
		urb->transfer_buffer_length = pdu->u.cmd_submit.transfer_buffer_length;
		/*
		urb->bandwidth              = pdu->u.cmd_submit.bandwidth;

			hided by TengFei, 10.10.20
		*/
		urb->start_frame            = pdu->u.cmd_submit.start_frame;
		urb->number_of_packets      = pdu->u.cmd_submit.number_of_packets;
		urb->interval               = pdu->u.cmd_submit.interval;
	}
}
...
static void usbip_pack_return_pdu(struct usbip_header *pdu, struct urb *urb, int pack)
{
	if (pack) {
		/* stub_tx.c */
		/*pdu->u.ret_submit.transfer_flags	= urb->transfer_flags;*/
		pdu->u.ret_submit.status		= urb->status;
		pdu->u.ret_submit.actual_length	= urb->actual_length;
		/*pdu->u.ret_submit.bandwidth		= urb->bandwidth;*/
		pdu->u.ret_submit.start_frame		= urb->start_frame;
		pdu->u.ret_submit.error_count	= urb->error_count;
		/*pdu->u.ret_submit.number_of_packets	= urb->number_of_packets;*/
	} else {
		/* vhci_rx.c */
		/*urb->transfer_flags		= pdu->u.ret_submit.transfer_flags;*/
		urb->status			= pdu->u.ret_submit.status;
		urb->actual_length		= pdu->u.ret_submit.actual_length;
		/*urb->bandwidth			= pdu->u.ret_submit.bandwidth;*/
		urb->start_frame		= pdu->u.ret_submit.start_frame;

		urb->error_count		= pdu->u.ret_submit.error_count;
	}
}
