/*
 * $Id: vhci_tx.c 100 2006-03-31 18:39:31Z taka-hir $
 *
...
static void setup_cmd_submit_pdu(struct usbip_header *pdup,  struct urb *urb)
{
	...
	int busnum = vdev->busnum;
	int devnum = vdev->devnum;
	pdup->base.seqnum = priv->seqnum;
	pdup->base.devid   = (busnum<<16) | devnum;
...
static int vhci_send_cmd_unlink(struct vhci_device *vdev)
{
	...
	while ((unlink = dequeue_from_unlink_tx(vdev)) != NULL) {
		...
		int busnum  = vdev->busnum;
		int devnum  = vdev->devnum;
		pdu_header.base.devid	= (busnum<<16) | devnum;
...
void vhci_tx_loop(struct usbip_task *ut)
{
	...
	while (1) {
		...
		wait_event_interruptible(vdev->waitq, 
				(!list_empty(&vdev->priv_tx) || !list_empty(&vdev->unlink_tx) ));
