/*
 * $Id: vhci_rx.c 101 2006-05-16 13:52:41Z taka-hir $
 *
...
static void vhci_recv_submit(struct vhci_device *vdev, struct usbip_header *pdu)
{
...
static void vhci_recv_unlink(struct vhci_device *vdev, struct usbip_header *pdu)
{
	...
	if (!urb) {
		 ...
		 * already recieved the result of its submit result and gave
		 * back the URB.
		 */
...
static void vhci_rx_pdu(struct usbip_device *ud)
{
	...
	switch(pdu.base.command) {
		case USBIP_RET_SUBMIT:
			vhci_recv_submit(vdev, &pdu);
			break;

		case USBIP_RET_UNLINK:
			vhci_recv_unlink(vdev, &pdu);
			break;
		default:
			/* NOTREACHED */
			uerr("unknown pdu %u\n", pdu.base.command);
