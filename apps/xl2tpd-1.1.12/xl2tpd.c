...
int start_pppd (struct call *c, struct ppp_opts *opts)
{
	...
	//add by yangxv for three parameters, 2008.04.29
	...
	stropt[pos++] = strdup("httpd-pid");
	stropt[pos++] = strdup(gconfig.tid);
...
#else   
  	  stropt[pos++] = strdup ("pppol2tp_init");
#endif
...
void init_args(int argc, char *argv[])
{
    ...
    for (i = 1; i < argc; i++) {
		...
		//add three parameters(user,password,mtu) by yangxv, 2008.04.29
		else if (! strncmp(argv[i], "user", 4)) {
			if (++i == argc)
				usage();
			else
				strncpy(gconfig.user, argv[i],
						sizeof(gconfig.user) - 1);
		}
		else if (! strncmp(argv[i], "password", 8)) {
			if (++i == argc)
				usage();
			else
				strncpy(gconfig.password, argv[i],
						sizeof(gconfig.password) - 1);
		}
		else if (! strncmp(argv[i], "mtu", 3)) {
			if (++i == argc)
				usage();
			else
				strncpy(gconfig.mtu, argv[i],
						sizeof(gconfig.mtu) - 1);
		}
		else if (! strncmp(argv[i], "httpd-pid", 9)) {
			if (++i == argc)
				usage();
			else
				strncpy(gconfig.tid, argv[i],
						sizeof(gconfig.tid) - 1);
		}
		//end add
        else {
            usage();
