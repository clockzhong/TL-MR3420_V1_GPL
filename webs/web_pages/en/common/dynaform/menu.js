...
//	directory level: 0: directory has no branches;  1: directory level 1;  2:level 2;   and so on
	...
	"VlanTagCfgRpm",				0,		2,			str_menu.ntwVlanTagCfg,
	"PortMirrorCfgRpm",			0,		2,			str_menu.ntwPortMirrorCfg,
	...
	"CertManageRpm", 			0,		2, 		str_menu.certManage,
	...
	"Tr069CfgRpm",				0,		0,			str_menu.tr069,
	...
	"LanIPv6CfgRpm",				0,		2,		str_menu.ipv6Lan,
	...
	"SuperLoginPwdRpm",			0,		2,			str_menu.superPassword,
...
function menuDisplay()
{
	var i = 0;
	var className;
	var superVlanSetting = "VlanTagCfgRpm";
	var portMirrorSetting = "PortMirrorCfgRpm";
	var tr069CfgSetting = "Tr069CfgRpm";
	for (var n = 0; n < menuList.length; n +=4  )
	{
		if ( menuList[n+1] != 1 )
		{
		...
		if ((superVlanSetting == menuList[n])
			|| (portMirrorSetting == menuList[n])
			|| (tr069CfgSetting == menuList[n]) )
		{
			document.write('<ol id=ol'+i+' class='+className+' style="display:'+display+'; background-position:2px;PADDING-LEFT:2px;"><A id=a'
			+ i +' href="/adminRpm/'+menuList[n]+'.htm" target=mainFrame class=L1 onClick="doClick('+i+');">'+((power > 0)? '- ':'')+menuList[n+3]+'</a></ol>');
		}
		else
		{
			document.write('<ol id=ol'+i+' class='+className+' style="display:'+display+'; background-position:2px;PADDING-LEFT:2px;"><A id=a'
			+ i +' href="/userRpm/'+menuList[n]+'.htm" target=mainFrame class=L1 onClick="doClick('+i+');">'+((power > 0)? '- ':'')+menuList[n+3]+'</a></ol>');
		}
