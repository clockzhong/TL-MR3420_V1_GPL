        ...
        )
{
		...
		bss->ssid.ssid_len = strlen(value) - 2;/* changed by song, avoid negore blank */
		if (bss->ssid.ssid_len > HOSTAPD_MAX_SSID_LEN ||
			...
			if (!strcasecmp(value,"any")) {
				...
				bss->ssid.ssid[0] = '\0';
					...
					memcpy(bss->ssid.ssid, value + 1,
					bss->ssid.ssid_len);  /* Changed by songwei to copy ssid */
			}
	...
	if (bss->auth_algs == 0) {
		...
		bss->wpa_gmk_rekey = atoi(value);
               ...
               /* Changed by songwei, 20090917, to strip tag */
		char tmpPassphrase[64];
		int len = strlen(value) - 2;
		strncpy(tmpPassphrase, value + 1, len);
		tmpPassphrase[len] = '\0';
		...
		if (len < 8 || len > 63) {
			...
			if ((bss->ssid.wpa_passphrase = strdup(tmpPassphrase)) == NULL)
                                goto malloc_failure;
		}
		/* End change */
