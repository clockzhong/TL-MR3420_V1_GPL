                             ...
                             unsigned long fis_origin)
{
	...
	for (i = 0; i < sizeof(flash_sectors)/sizeof(struct fis_image_desc); i ++)
	{
		...
		{
			/* art is the last sector, flash size is 4MB aligned -- lsz, 081211 */
			flash_sectors[i].flash_base = ((tag.totalImageLen + 0x3FFFFF) & 0xFFC00000)-0x10000;
			flash_sectors[i].size = 0x10000;	/* config size */
		}
	}
	/************add by zhangwu 24May07********************/
	buf = flash_sectors;
	printk(KERN_NOTICE "Searching for RedBoot partition table\n");
	/******************************************************/
	...
	for (i = 0; i < numslots; i++) {
		...
		if (!memcmp(buf[i].name, "Config", strlen("Config")))
	//	if (!memcmp(buf[i].name, "pb42-jffs2", 11))
			break;
	}
	if (i == numslots) {
		/* Didn't find it */
...
//	vfree(buf);//now no need,del by zhangwu 24May07
	return ret;
}
