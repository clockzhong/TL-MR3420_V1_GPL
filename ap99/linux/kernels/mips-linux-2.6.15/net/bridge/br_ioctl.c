 ...
 *
 *	$Id: //depot/sw/releases/7.3_AP/linux/kernels/mips-linux-2.6.15/net/bridge/br_ioctl.c#1 $
 *
...
int wlanwds_enable = 0;//add for awareness of wlan wds status

static int br_set_wlanwds_state(struct net_bridge *br, unsigned long state)
{
	if(state)
		wlanwds_enable = 1;
	else
...
static int old_dev_ioctl(struct net_device *dev, struct ifreq *rq, int cmd)
{
	...
	case BRCTL_SET_WLANWDS:
		return br_set_wlanwds_state(br, args[1]);
