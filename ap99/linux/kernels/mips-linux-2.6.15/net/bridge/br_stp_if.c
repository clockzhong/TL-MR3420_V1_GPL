...
void br_stp_recalculate_bridge_id(struct net_bridge *br)
{
	...
	const unsigned char *ath0_addr = br_mac_zero;
	const unsigned char *eth0_addr = br_mac_zero;
	
	list_for_each_entry(p, &br->port_list, list) {
		if (!strcmp(p->dev->name, "ath0"))		/* record ath0's mac */
			ath0_addr = p->dev->dev_addr;
		else if (!strcmp(p->dev->name, "eth0"))	/* record eth0's mac */
	...
	if (ath0_addr != br_mac_zero)		/* if ath0 exists, use ath0's mac */
		addr = ath0_addr;
	else if (eth0_addr != br_mac_zero)	/* no ath0, use eth0's mac */
		addr = eth0_addr;
	/* no eth0, use the max one */
