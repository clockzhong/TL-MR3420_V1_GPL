			...
			const char **dptr)
{
	...
	if (memcmp(*dptr, "SIP/2.0", sizeof("SIP/2.0") - 1) == 0) {
		...
		/* a response from within the LAN, replace CONTACT 
		 * with GW's WAN IP */
		if ((ctinfo) < IP_CT_IS_REPLY) {
		...
		/* a response from WAN, replace VIA with original LAN IP */
		...
		/* a REGISTER response from WAN, replace CONTACT with original LAN IP */
	...
	if ((ctinfo) < IP_CT_IS_REPLY) {
		/* a request from LAN, replace VIA with GW's WAN IP */
		if (!mangle_sip_packet(pskb, ctinfo, ct, dptr, (*pskb)->len - dataoff,
		    buffer, bufflen, &ct_sip_hdrs[POS_VIA])) {
			return 0;
		}

		/* replace CONTACT with GW's WAN IP */
	...
	/* a request from WAN, replace the IP in the header with original LAN IP */
