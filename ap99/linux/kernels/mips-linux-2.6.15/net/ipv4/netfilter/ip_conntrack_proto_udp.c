...
unsigned long ip_ct_udp_timeout = 60*HZ;
unsigned long ip_ct_udp_timeout_stream = 120*HZ;

unsigned long ip_ct_udp_dns_replied_timeout = 20*HZ;
unsigned long ip_ct_udp_dns_unreplied_timeout = 3*HZ;
			     ...
			     struct ip_conntrack_tuple *tuple)
{
	...
	/* Change Symmetric NAT of UDP to Full Cone NAT -- lsz 09.04.23 */
	//printk("IF:%s src:%X dst:%X\n", skb->input_dev->name, skb->nh.iph->saddr, skb->nh.iph->daddr);
	#if 1
	tuple->dst.u.udp.port = hp->dest;
	#else
	if (skb->input_dev->name && 
		(!strcmp(skb->input_dev->name, "ppp0")||
		!strcmp(skb->input_dev->name, "eth1") ||
		!strcmp(skb->input_dev->name, "eth2") ))
	{
		tuple->src.u.udp.port = 0;
		tuple->src.ip = 0;
	}
	else
	{
		      ...
		      enum ip_conntrack_info ctinfo)
{
	...
	if (test_bit(IPS_SEEN_REPLY_BIT, &conntrack->status)) {
		...
		/* If this is a dns connection, shorten timeout--by lsz 04Oct07 */		
		if (dport == 53 || sport == 53)
			ip_ct_refresh_acct(conntrack, ctinfo, skb, ip_ct_udp_dns_replied_timeout);
		else
			ip_ct_refresh_acct(conntrack, ctinfo, skb, ip_ct_udp_timeout_stream);
	...
	else
	{
		...
		*/
		if (dport == 53)
			ip_ct_refresh_acct(conntrack, ctinfo, skb, ip_ct_udp_dns_unreplied_timeout);
		else   
