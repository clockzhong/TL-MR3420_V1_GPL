...
DECLARE_RWLOCK(ip_conntrack_expect_tuple_lock); /* lmc added for RTSP */
void (*ip_conntrack_rtsp_data_ports_release_hook)(struct ip_conntrack *ct)= NULL;
EXPORT_SYMBOL_GPL(ip_conntrack_rtsp_data_ports_release_hook);
...
clean_from_lists(struct ip_conntrack *ct)
{
	...
	/* lmc added this to release the rtsp_data_ports table, the client which exited abnormally won't transmit TEARDOWN message */
	if (ip_conntrack_rtsp_data_ports_release_hook)
	{
		ip_conntrack_rtsp_data_ports_release_hook(ct);
	}
			     ...
			     int (*okfn)(struct sk_buff *))
{
	...
	if ((*pskb)->nfct) {
		...
		/* Change to Drop by lsz 080617, if accept, it'll result in NO-NAT-PASS, but We are a NAT Router */
		/* Change to Accept by lsz 080627, if drop, the QSS netlink pkt will be dropped and QSS function unavailable */
		return NF_ACCEPT;
	}
...
/* Doesn't cover locally-generated broadcast, so not worth it. */
	...
	if (!(ct = resolve_normal_ct(*pskb, proto,&set_reply,hooknum,&ctinfo))) {
		...
		/* Modify by lsz 080515, if accept, it'll result in NO-NAT-PASS, but We are a NAT Router */
...
int __init ip_conntrack_init(void)
{
	...
	/* 28Sep07, lishaozhang change max to 5K for WR841N */
	ip_conntrack_max = 1024 * 5;
