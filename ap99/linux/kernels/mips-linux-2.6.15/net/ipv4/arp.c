 ...
 *
 * Version:	$Id: //depot/sw/releases/7.3_AP/linux/kernels/mips-linux-2.6.15/net/ipv4/arp.c#1 $
 *
...
char *mac2asc(char *buffer, unsigned char *ha, unsigned char addr_len) {
	...
	if (hatype == ARPHRD_AX25 || hatype == ARPHRD_NETROM)
		...
		for (k=0,j=0;k<HBUFFERLEN-3 && j<addr_len;j++) {
			buffer[k++]=hexbuf[(ha[j]>>4)&15 ];
			buffer[k++]=hexbuf[ha[j]&15     ];
			buffer[k++]=':';
	}
		buffer[--k]=0;
				   ...
				   struct neighbour *n)
{
	...
	seq_printf(seq, "%-16s 0x%-10x0x%-10x%s     *        %s\n",
		   ...
		   mac2asc(hbuffer,n->ha,dev->addr_len), dev->name);
	read_unlock(&n->lock);
}
